...
- Since the DEgg's properties are closer to what is to be simulated we used its radius and reference area and altered the **angular sensitivity** and **wavelength acceptance** in a way that:
   - We needed the product <br />
     ...
     For the **lowerhalf** option for the sensor: <br/> 
     when $\theta=0$, the maximum cross section is achieved, whereas when $\theta=\pi$ the result is $0$. Since only the lower part of the detector is active, a photon arriving with a direction vector perpedincular to the maximum cross section area and pointing towards it, it won't be detected by that sensor. <br />
